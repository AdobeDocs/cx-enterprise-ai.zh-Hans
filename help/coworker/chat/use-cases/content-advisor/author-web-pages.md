---
title: 与同事一起创作AEM Sites页面
description: 了解如何使用Adobe CX Enterprise Coworker创作AEM Sites页面，方法是使用自然语言添加从可视化内容片段构建的新促销活动。
version: Experience Manager as a Cloud Service
role: Leader, User, Developer
level: Beginner
doc-type: Tutorial
duration: 302
last-substantial-update: 2026-09-25T00:00:00.000Z
product_v2:
  - id: fd1f54a9-f50c-467d-8956-cebbaf4f3eb8
    internal-label: Experience Manager
topic_v2:
  - id: bce87dde-a4ab-44c9-8a18-ad66e4ddb377
    internal-label: Customer experience
source-git-commit: 93cccf40d67507df59e07ed016253dbf1d6b3b2e
workflow-type: tm+mt
source-wordcount: '133'
ht-degree: 0%
---
# 与同事一起创作AEM Sites页面

使用Adobe的对话式人工智能[同事](./overview.md)以纯语言描述您想要的内容，而不是通过页面编辑器进行点击，来创作AEM Sites页面。

在此视频中，Co-worker使用聊天提示向WKND主页添加了新的促销活动，该主页从可视化内容片段构建。

>[!VIDEO](https://video.tv.adobe.com/v/3503863/?learn=on)

>[!NOTE]
>
>本视频演示了同事在基于页面编辑器的网站上创作页面的过程。 在与Edge Delivery Services一起交付的网站上，同事的工作方式相同。

请参阅[同事聊天概述](https://experienceleague.adobe.com/zh-hans/docs/cx-enterprise-ai/experience-cloud-ai/coworker/chat/overview)以开始使用同事聊天。
